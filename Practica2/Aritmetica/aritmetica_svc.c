/*
 * Please do not edit this file.
 * It was generated using rpcgen.
 */

#include "aritmetica.h"
#include <stdio.h>
#include <stdlib.h>
#include <rpc/pmap_clnt.h>
#include <string.h>
#include <memory.h>
#include <sys/socket.h>
#include <netinet/in.h>

#ifndef SIG_PF
#define SIG_PF void(*)(int)
#endif

static int *
_sum_1 (sum_1_argument *argp, struct svc_req *rqstp)
{
	return (sum_1_svc(argp->arg1, argp->arg2, rqstp));
}

static int *
_mul_1 (mul_1_argument *argp, struct svc_req *rqstp)
{
	return (mul_1_svc(argp->arg1, argp->arg2, rqstp));
}

static int *
_res_1 (res_1_argument *argp, struct svc_req *rqstp)
{
	return (res_1_svc(argp->arg1, argp->arg2, rqstp));
}

static int *
_div_1 (div_1_argument *argp, struct svc_req *rqstp)
{
	return (div_1_svc(argp->arg1, argp->arg2, rqstp));
}

static void
practica1_1(struct svc_req *rqstp, register SVCXPRT *transp)
{
	union {
		sum_1_argument sum_1_arg;
		mul_1_argument mul_1_arg;
		res_1_argument res_1_arg;
		div_1_argument div_1_arg;
	} argument;
	char *result;
	xdrproc_t _xdr_argument, _xdr_result;
	char *(*local)(char *, struct svc_req *);

	switch (rqstp->rq_proc) {
	case NULLPROC:
		(void) svc_sendreply (transp, (xdrproc_t) xdr_void, (char *)NULL);
		return;

	case sum:
		_xdr_argument = (xdrproc_t) xdr_sum_1_argument;
		_xdr_result = (xdrproc_t) xdr_int;
		local = (char *(*)(char *, struct svc_req *)) _sum_1;
		break;

	case mul:
		_xdr_argument = (xdrproc_t) xdr_mul_1_argument;
		_xdr_result = (xdrproc_t) xdr_int;
		local = (char *(*)(char *, struct svc_req *)) _mul_1;
		break;

	case res:
		_xdr_argument = (xdrproc_t) xdr_res_1_argument;
		_xdr_result = (xdrproc_t) xdr_int;
		local = (char *(*)(char *, struct svc_req *)) _res_1;
		break;

	case div:
		_xdr_argument = (xdrproc_t) xdr_div_1_argument;
		_xdr_result = (xdrproc_t) xdr_int;
		local = (char *(*)(char *, struct svc_req *)) _div_1;
		break;

	default:
		svcerr_noproc (transp);
		return;
	}
	memset ((char *)&argument, 0, sizeof (argument));
	if (!svc_getargs (transp, (xdrproc_t) _xdr_argument, (caddr_t) &argument)) {
		svcerr_decode (transp);
		return;
	}
	result = (*local)((char *)&argument, rqstp);
	if (result != NULL && !svc_sendreply(transp, (xdrproc_t) _xdr_result, result)) {
		svcerr_systemerr (transp);
	}
	if (!svc_freeargs (transp, (xdrproc_t) _xdr_argument, (caddr_t) &argument)) {
		fprintf (stderr, "%s", "unable to free arguments");
		exit (1);
	}
	return;
}

int
main (int argc, char **argv)
{
	register SVCXPRT *transp;

	pmap_unset (PRACTICA1, VERSIONACTOCA1);

	transp = svcudp_create(RPC_ANYSOCK);
	if (transp == NULL) {
		fprintf (stderr, "%s", "cannot create udp service.");
		exit(1);
	}
	if (!svc_register(transp, PRACTICA1, VERSIONACTOCA1, practica1_1, IPPROTO_UDP)) {
		fprintf (stderr, "%s", "unable to register (PRACTICA1, VERSIONACTOCA1, udp).");
		exit(1);
	}

	transp = svctcp_create(RPC_ANYSOCK, 0, 0);
	if (transp == NULL) {
		fprintf (stderr, "%s", "cannot create tcp service.");
		exit(1);
	}
	if (!svc_register(transp, PRACTICA1, VERSIONACTOCA1, practica1_1, IPPROTO_TCP)) {
		fprintf (stderr, "%s", "unable to register (PRACTICA1, VERSIONACTOCA1, tcp).");
		exit(1);
	}

	svc_run ();
	fprintf (stderr, "%s", "svc_run returned");
	exit (1);
	/* NOTREACHED */
}
